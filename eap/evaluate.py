...
def evaluate_graph_multitoken(model: HookedTransformer, 
                               graph: Graph, 
                               df: pd.DataFrame, 
    ...
    graph.prune()
    in_graph_matrix = 1 - graph.in_graph.to(device=model.cfg.device, dtype=model.cfg.dtype)
    neuron_matrix = 1 - graph.neurons_in_graph.to(device=model.cfg.device, dtype=model.cfg.dtype) if graph.neurons_in_graph is not None else None
    ...
    # group by label length
    groups = defaultdict(list)
    for i, row in df.iterrows():
        correct = safe_parse(row["correct_idx"])
        incorrect = safe_parse(row["incorrect_idx"])
        if len(correct) == len(incorrect):
            groups[len(correct)].append(i)

    group_iter = groups.items() if quiet else tqdm(groups.items(), desc="Evaluating groups")

    for label_len, indices in group_iter:
        group_df = df.iloc[indices]
        dataset = EAPDataset(group_df)
        dataloader = dataset.to_dataloader(batch_size=batch_size)

        for clean_batch, corrupted_batch, labels in dataloader:
            label_length = len(labels[0][0])

            for token_step in range(label_length):
                current_label = (
                    [correct[token_step] for correct in labels[0]],
                    [incorrect[token_step] for incorrect in labels[1]]
                )
                clean_texts = [f"{c} {model.to_string(correct[:token_step])}" for c, correct in zip(clean_batch, labels[0])]
                corrupted_texts = [f"{c} {model.to_string(incorrect[:token_step])}" for c, incorrect in zip(corrupted_batch, labels[1])]

                clean_tokens, attention_mask, input_lengths, n_pos = tokenize_plus(model, clean_texts)
                corrupted_tokens, _, _, _ = tokenize_plus(model, corrupted_texts)

                (fwd_hooks_corrupted, fwd_hooks_clean, _), activation_diff = make_hooks_and_matrices(
                    model, graph, len(clean_batch), n_pos, None)

                input_hooks = make_input_construction_hooks(activation_diff, in_graph_matrix, neuron_matrix)
                ...
                with torch.inference_mode():
                    if intervention == 'patching':
                        ...
                        activation_diff += means

                    clean_logits = None if skip_clean else model(clean_tokens, attention_mask=attention_mask)

                    with model.hooks(fwd_hooks_clean + input_hooks):
                        logits = model(clean_tokens, attention_mask=attention_mask)

                for i, metric in enumerate(metrics):
                    r = metric(logits, clean_logits, input_lengths, current_label).cpu()
                    if len(r.size()) == 0:
                        r = r.unsqueeze(0)
                    results[i].append(r)

    results = [torch.cat(r) for r in results]
    return results[0].mean() if len(results) == 1 else torch.stack(results).mean()
...
def evaluate_baseline_multitoken(model: HookedTransformer, 
                                 df: pd.DataFrame, 
    ...
    for label_len, indices in group_iter:
        ...
        for clean_batch, corrupted_batch, label in dataloader:
            ...
            for token_step in range(label_len):
                ...
                current_label = (
                    [correct[token_step] for correct in label[0]],
                    [incorrect[token_step] for incorrect in label[1]]
                )

                clean_tokens, attn_mask, input_lengths, _ = tokenize_plus(model, current_clean)
                corrupted_tokens, _, _, _ = tokenize_plus(model, current_corrupted)

                with torch.inference_mode():
                    logits_clean = model(clean_tokens, attention_mask=attn_mask)
                    logits_corrupted = model(corrupted_tokens, attention_mask=attn_mask)

                for i, metric in enumerate(metrics):
                    r = metric(
                        logits_corrupted if run_corrupted else logits_clean,
                        logits_clean,
                        input_lengths,
                        current_label
                    ).cpu()
    ...
    if not quiet:
        print(f"✔ Evaluated {total_items} total tokens.")
